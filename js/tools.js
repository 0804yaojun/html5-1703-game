 ...
 *		options = {
 *			url:"", 请求资源路径
 *			type:"", 请求方式，get或post，默认为 get
 *			data:{action:"", nickname:"", message:""}, 向服务器传递的数据
 *			dataType:"json", 预期从服务器返回数据的类型，可取 text | json
 *			success:function(data){}, 请求成功执行的函数,函数参数data表示响应数据
...
function ajax(options) {
	...
	if (!url) // 未设置请求资源URL，则结束函数执行
		return;
	var method = options.type || "get"; // 请求方式
	var queryString = null; // 存放查询字符串的变量

	if (options.data) { // 有向服务器传递的数据
		queryString = [];
		for (var attr in options.data) {
			queryString.push(attr + "=" + options.data[attr]);
		}
		// ["action=xx", "nickname=xxx", "message=xxxxx"]
		queryString = queryString.join("&");
		// "action=xx&nickname=xxx&message=xxxxx"
	}

	// 如果请求方式为 get ，则将查询字符串连接在URL后
